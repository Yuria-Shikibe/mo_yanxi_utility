...
public:
	constexpr explicit(false) concave_curve(float xm = 0.5, PowType p = PowType(2)) : xm_(xm), p_(p){
		if(xm_ <= 0.0 || xm_ >= 1.0){
			throw std::invalid_argument("0 < xm < 1");
		}
		if(p_ <= 0.0){
			throw std::invalid_argument("p must > 0");
		}
	}

	constexpr float operator()(float x) const noexcept{
		if constexpr(std::integral<PowType>){
			if(x <= xm_){
				return math::pow_integral((xm_ - x) / xm_, p_);
			} else{
				return math::pow_integral((x - xm_) / (1.f - xm_), p_);
			}
		} else{
			if(x <= xm_){
				return std::pow((xm_ - x) / xm_, p_);
			} else{
				return std::pow((x - xm_) / (1.f - xm_), p_);
			}
		}
	}
};
...
export
template <unsigned PowCount = 2>
	requires (PowCount > 0)
class concave_curve_fixed{
private:
	float xm_;
	float xm_l_;
	float xm_r_;
...
public:
	constexpr explicit concave_curve_fixed(float xm = 0.5f) : xm_(xm), xm_l_(1.f / xm), xm_r_(-1.f / (1.f - xm)){
		if(xm_ <= 0.0f || xm >= 1.0f){
			throw std::invalid_argument("0 < xm < 1");
		}
	}
	...
	constexpr float operator()(float x) const noexcept{
		return math::pow_integral<PowCount>((xm_ - x) * ((x <= xm_) ? xm_l_ : xm_r_));
	}
};
...
export{
	...
	inline constexpr auto linear_margined = interp_func{
			[](const float x) static noexcept{
				return math::fma(x, margin_out - margin_in, margin_in);
			}
		};

	template <float margin_in, float margin_out = 1.f>
	inline constexpr auto linear_map = interp_func{
			[](const float x) static noexcept{
